...
export async function buildProject(projectPath: string): Promise<string> {
  ...
  const config = await parseConfig(projectPath)
  const html = await renderHTML(path.resolve(projectPath, config.template.root))
...
export async function parseConfig(projectPath: string): Promise<PackpressConfig> {
  ...
  if (object.template && object.template.root) {
    return object
  } else {
