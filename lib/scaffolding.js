...
export async function createNewProject(projectPath, options = {}) {
  ...
  const templateFiles = await template.getFiles(projectPath)

  return Promise.all(templateFiles.map(async function(file) {
